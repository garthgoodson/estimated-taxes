...
describe('quarter API boundary', () => {
  ...
  it('rejects malformed complete-quarter responses', () => {
    ...
    expect(isQuarterResource({ ...resource(), result: { ...resource().result, annual_summary: { ...resource().result.annual_summary, spouses: [{}] } } })).toBe(false)
  })
})
