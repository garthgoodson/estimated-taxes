...
namespace { void require(bool v,const char*m){if(!v)throw std::runtime_error(m);} ActiveRules rules(){auto f=official_federal_rules();auto c=official_california_rules();return {{1,Jurisdiction::federal,true,false,{},f,{}},{2,Jurisdiction::california,true,false,{},{},c},false,false};} CalculationSnapshot sample(const std::string&name){CalculationSnapshot s;s.name=name;s.as_of_date="2026-03-31";s.rules=rules();for(int i=0;i<4;++i)s.inputs.quarters[i].quarter=i+1;s.inputs.quarters[0].federal_payment={123,"2026-03-01"};s.inputs.quarters[0].spouse_1_paystub=PaystubSnapshot{"2026-03-01","monthly",100,0,10,5,300,300,30,15,0,0,0,"2026-10-31"};s.taxes.federal.details.annual_liability_cents=1000;s.recommendations.federal.current_quarter=1;return s;} std::string path(){auto p=std::filesystem::temp_directory_path()/"estimated_taxes_snapshot_test.sqlite";std::filesystem::remove(p);return p.string();}
void round_trip_operations(){const auto p=path();SnapshotStore store(p);auto first=store.save(sample("first"));auto second=store.save(sample("second"));require(store.list().size()==2,"list snapshots");auto loaded=store.load(first.id);require(loaded.name=="first"&&loaded.as_of_date=="2026-03-31"&&loaded.inputs.quarters[0].federal_payment.amount_cents==123&&loaded.inputs.quarters[0].spouse_1_paystub->projection_end_date=="2026-10-31"&&loaded.rules.federal.federal==official_federal_rules(),"complete round trip");store.rename(first.id,"renamed");loaded=store.load(first.id);require(loaded.name=="renamed"&&loaded.inputs.quarters[0].federal_payment.amount_cents==123,"rename changes only name");
 InputStore current(p); auto household=current.load_household(); household.spouse_1.label="Changed"; current.replace_household(household); RuleStore active(p); auto federal=official_federal_rules(); federal.standard_deduction_cents=1; active.replace_active(federal,official_california_rules()); loaded=store.load(first.id); require(loaded.household.spouse_1.label=="Spouse 1"&&loaded.rules.federal.federal==official_federal_rules(),"current inputs and rules do not rewrite snapshot");
 store.remove(second.id);require(store.list().size()==1&&store.load(first.id).name=="renamed","delete selected only");std::filesystem::remove(p);}
void migration_and_atomic_failure(){const auto p=path();{InputStore input(p);RuleStore rule(p);require(input.schema_version()==2,"prior schema");}SnapshotStore store(p);InputStore input(p);require(input.schema_version()==4,"projection end-date migration");bool failed{};try{(void)store.save(sample(""));}catch(const ValidationError&){failed=true;}require(failed&&store.list().empty(),"failed snapshot validation leaves no row");sqlite3* database{};require(sqlite3_open(p.c_str(),&database)==SQLITE_OK,"open test database");require(sqlite3_exec(database,"CREATE TRIGGER fail_snapshot BEFORE INSERT ON calculation_snapshots BEGIN SELECT RAISE(ABORT, 'forced failure'); END",nullptr,nullptr,nullptr)==SQLITE_OK,"create failing trigger");sqlite3_close(database);failed=false;try{(void)store.save(sample("forced"));}catch(const StorageError&){failed=true;}require(failed&&store.list().empty(),"failed snapshot persistence rolls back");std::filesystem::remove(p);}
